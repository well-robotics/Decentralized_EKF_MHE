...
void MHEproblem::setHorizon(int N, int dim_state, int dim_meas)
{
    this->N = N;
    ...
    I_corelate.resize(dim_state, dim_state);
    I_corelate.setIdentity();

    I_mag.resize(dim_state + dim_meas, dim_state + dim_meas);
    I_mag.setIdentity();
}
...
bool MHEproblem::updateQP(int T)
{
    ...
    vectorResize(lb_all, nConstraints);
    vectorResize(ub_all, nConstraints);
...
bool MHEproblem::marginalizeQP(int T)
{
    ...
    // int var_marginalize_size = 21 + 12 + 12 + 21;
    // int var_marginalize_size = 12 + 4 + 12 + 3;
    ...
    // var_idx_regs.erase(vc_marginalize_string);
    ...
    if (Cost_regs.count(dyn_marginalize_string))
    {
        ...
        //     VectorXd y_meas_marginalize = VectorXd::Zero(3);
        ...
        //     // std::cout << MatrixXd(M_p_inv) << std::endl;
        //     solver.compute(Q_dyn_marginalize);

        //     SparseMatrixType Q_dyn_marginalize_inv = solver.solve(I);

        //     // std::cout << R_meas_marginalize << std::endl;
        //     solver_4.compute(R_meas_marginalize);

        //     SparseMatrixType R_meas_marginalize_inv = solver_4.solve(I_4);

        //     SparseMatrix<double> A(16, 16);
        ...
        //     EigenUtils::SparseMatrixBlockAsign(A, 12, 12, A_22);
        //     EigenUtils::SparseMatrixBlockAsign(A, 0, 12, A_12);
        //     EigenUtils::SparseMatrixBlockAsign(A, 12, 0, A_21);

        //     // std::cout << "A" << MatrixXd(A) << std::endl;
        //     SparseMatrix<double> C = (-MatrixXd::Identity(12, 16)).sparseView();
        //     SparseMatrix<double> B = C.transpose();
        ...
        //     VectorXd u = VectorXd::Zero(16);
        //     u.segment(0, 12) = -b_dyn_marginalize + A_dyn_marginalize * M_p_inv * n_p;
        //     u.segment(12, 4) = y_meas_marginalize + H_meas_marginalize * M_p_inv * n_p;
        ...
        vectorResize(g_marginalize, nVar);
...
void MHEproblem::getsolution(int T)
{
    ...
    x_solution_now = solution.segment(var_idx_regs[x_name] - nVarStart, 15);
    std::cout << x_solution_now << std::endl;
    // std::cout << x_solution_now << std::endl;
    // Vector3d w_solution_now = solution.segment(var_idx_regs[w_name] - nVarStart, 3);
    // Vector3d realtive_lb = lb_all.segment(lb_all.size() - 3, 3);
    // Vector3d realtive_ub = ub_all.segment(lb_all.size() - 3, 3);

    // std::cout << w_solution_now << std::endl;
    // std::cout << "compare" << std::endl;
    // std::cout << realtive_lb << std::endl;
    // std::cout << "compare" << std::endl;
    // std::cout << realtive_ub << std::endl;
    // w_solution_now = solution.segment(var_idx_regs[w_name] - nVarStart, 21);
    // v_solution_now = solution.segment(var_idx_regs[v_name] - nVarStart, 12);
    // Vector3d v_solution_now = solution.segment(var_idx_regs[vcam_name] - nVarStart, 3);

    // std::cout << "Solution: \n"
    //           << x_solution_now << std::endl;
    // std::cout << "w Solution: \n"
    //           << w_solution_now << std::endl;
    // std::cout << "v Solution: \n"
    //           << v_solution_now << std::endl;
}
...
void MHEproblem::vectorResize(VectorXd &vector, const int new_size)
{
...
// auto stop_aq = std::chrono::time_point_cast<std::chrono::microseconds>(std::chrono::high_resolution_clock::now()).time_since_epoch().count();
// auto duration_aq = stop_aq - startap;
// std::cout << "ReduceOrder time elapsed: " << static_cast<double>(duration_aq) /1000000<< " seconds." << std::endl;
